  ...
  <li>official@jssstu.in</li>
      ...
      if ($result = mysqli_query($link, $sql)) {
        if (mysqli_num_rows($result) > 0) {
          ...
          while ($row1 = mysqli_fetch_array($result)) {
            ...
            echo "<input type='radio' id='male' name='sgender' value='MALE'";
            if ("{$row1['SGENDER']}" == 'MALE') echo "checked=checked";
            echo ">";
            echo "<label for='male'> Male</label>";
            echo "<input type='radio' id='female' name='sgender' value='FEMALE'";
            if ("{$row1['SGENDER']}" == 'FEMALE') echo "checked=checked";
            echo ">";
            echo "<label for='female'> Female</label>";
            echo "<input type='radio' id='other' name='sgender' value='OTHER'";
            if ("{$row1['SGENDER']}" == 'OTHER') echo "checked=checked";
            echo ">";
            ...
            echo "<option value='D01'";
            if ("{$row1['DID']}" == 'D01') {
              echo "selected";
            };
            echo ">Computer Science and Engineering</option>";
            echo "<option value='D02'";
            if ("{$row1['DID']}" == 'D02') {
              echo "selected";
            };
            echo ">Electronics and Communication Engineering</option>";
            echo "<option value='D04'";
            if ("{$row1['DID']}" == 'D04') {
              echo "selected";
            };
            echo ">Information Science and Engineering</option>";
            echo "<option value='D03'";
            if ("{$row1['DID']}" == 'D03') {
              echo "selected";
            };
            ...
            echo ">No,Localite</option>";
            echo "<option value='H001'";
            if ("{$row1['HID']}" == 'H001') {
              echo "selected";
            };
            echo ">A Block</option>";
            echo "<option value='H002'";
            if ("{$row1['HID']}" == 'H002') {
              echo "selected";
            };
            echo ">H Block</option>";
            echo "<option value='H003'";
            if ("{$row1['HID']}" == 'H003') {
              echo "selected";
            };
            echo ">GH Block</option>";
            echo "<option value='H004'";
            if ("{$row1['HID']}" == 'H004') {
              echo "selected";
            };
            echo ">Mess Block</option>";
            echo "</select>";
            echo "</div>";
    ...
    <label for=''>Seat:</label>
        ...
        <select name='seatid' required>
          ...
          <option value='E021'";
            if ("{$row1['SEATID']}" == 'E021') echo "selected";
            echo ">Aided</option>
          <option value='E057'";
            if ("{$row1['SEATID']}" == 'E057') echo "selected";
            echo ">Unaided</option>
          <option value='E060'";
            if ("{$row1['SEATID']}" == 'E060') echo "selected";
            echo ">COMED-K</option>
          <option value='E070'";
            if ("{$row1['SEATID']}" == 'E070') echo "selected";
            echo ">Payment</option>
          <option value='E080'";
            if ("{$row1['SEATID']}" == 'E080') echo "selected";
            echo ">Others</option>
        </select>
      </div>
      <div class='sele'>
        <p>Payment Method:</p>
        <input type='radio' id='dd' name='paymethod' value='DD'";
            if ("{$row1['PAYMENTMETHOD']}" == 'DD') echo "checked";
            echo ">
        <label for='dd'>DD</label>
        <input type='radio' id='neft' name='paymethod' value='NEFT'";
            if ("{$row1['PAYMENTMETHOD']}" == 'NEFT') echo "checked";
            echo ">
        <label for='neft'>NEFT</label>
        <input type='radio' id='others' name='paymethod' value='Others'";
            if ("{$row1['PAYMENTMETHOD']}" == 'Others') echo "checked";
            echo ">
